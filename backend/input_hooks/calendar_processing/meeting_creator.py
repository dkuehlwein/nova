...
class MeetingCreator:
    ...
    async def create_prep_meeting(self, original_meeting: CalendarMeetingInfo, 
        ...
        try:
            ...
            # Find calendar create event tool (prefixed per ADR-015)
            for tool in tools:
                if hasattr(tool, 'name'):
                    # Match google_workspace-create_event (ADR-015 prefixed name)
                    if tool.name == 'google_workspace-create_event':
                        create_event_tool = tool
                        break
                    # Fallback: any tool with create and event in the name
    ...
    async def check_prep_meeting_exists(self, original_meeting: CalendarMeetingInfo, 
        ...
        try:
            ...
            for tool in tools:
                if hasattr(tool, 'name'):
                    # Match google_workspace-list_events (ADR-015 prefixed name)
                    if tool.name == 'google_workspace-list_events':
                        list_events_tool = tool
                        break
                    # Fallback: any tool with list and event in the name
            ...
            # Create a base title pattern without UUIDs or test IDs
            base_title = original_meeting.title
            # Remove common test patterns like "E2E Test", timestamps, UUIDs
            import re
            base_title_clean = re.sub(r'\s+(E2E|Test|\d{10,}|[a-f0-9]{8})\b.*$', '', base_title, flags=re.IGNORECASE)
            base_title_clean = base_title_clean.strip()
            
            expected_title_pattern = f"{self.prep_title_prefix} {base_title_clean}"
    ...
    async def update_prep_meeting(self, prep_event_id: str, 
        ...
        try:
            ...
            # Find calendar update event tool (prefixed per ADR-015)
            for tool in tools:
                if hasattr(tool, 'name'):
                    # Match google_workspace-update_event (ADR-015 prefixed name)
                    if tool.name == 'google_workspace-update_event':
                        update_event_tool = tool
                        break
                    # Fallback: any tool with update and event in the name
                    if 'update' in tool.name.lower() and 'event' in tool.name.lower():
                        update_event_tool = tool
    ...
    def format_memo_for_description(self, memo_text: str, original_meeting: CalendarMeetingInfo, 
        ...
        Args:
            ...
            thread_id: Optional LangGraph thread ID for chat link generation
            
        Returns:
            Formatted description for calendar event with chat link
        """
        try:
            ...
            # Add chat link if thread_id is provided
            if thread_id:
                chat_url = f"{settings.FRONTEND_BASE_URL}/chat?thread={thread_id}"
                footer += f"\n\n💬 View AI conversation: {chat_url}"
