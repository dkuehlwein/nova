...
export function useChatPage() {
  ...
  const loadChats = useCallback(
    async (offset: number, isInitial: boolean = false, fallbackDecisions?: PendingDecision[]) => {
      try {
        if (!isInitial) {
  ...
  const handleChatSelect = useCallback(
    async (chatItem: ChatHistoryItem) => {
      // Set loading indicator immediately for visual feedback
      setLoadingChatId(chatItem.id);
      try {
        if (chatItem.task_id) {
  ...
  const handleDeleteChat = useCallback(
    async (chatItem: ChatHistoryItem, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const generateChatTitle = useCallback(async (threadId: string) => {
    try {
      ...
      if (result.title) {
        setChatHistory((prev) =>
          prev.map((ch) => (ch.id === threadId ? { ...ch, title: result.title } : ch)),
        );
      }
    } catch {
  ...
  const renameChat = useCallback(async (threadId: string, newTitle: string) => {
    try {
      ...
      setChatHistory((prev) =>
        prev.map((ch) => (ch.id === threadId ? { ...ch, title: newTitle } : ch)),
      );
    } catch (error) {
